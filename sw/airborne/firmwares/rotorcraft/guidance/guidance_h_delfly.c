...
/** @file firmwares/rotorcraft/guidance/guidance_h_delfly.c
 *  Horizontal guidance for the DelFly based on rotorcraft guidance_h.c.
 *
 */
...
int32_t guidance_lat_cmd_heading;

static void guidance_h_update_reference(void);
static void guidance_h_traj_run(bool_t in_flight);
static void guidance_lat_adjust_heading(bool_t in_flight);
...
void guidance_h_run(bool_t  in_flight)
{
  switch (guidance_h.mode) {
    ...
    case GUIDANCE_H_MODE_HOVER:
      ...
      /* compute heading */
      guidance_lat_adjust_heading(in_flight);
    ...
    case GUIDANCE_H_MODE_NAV:
      ...
      if (horizontal_mode == HORIZONTAL_MODE_ATTITUDE) {
        ...
        /* compute x,y earth commands */
        guidance_h_traj_run(in_flight);
        /* compute heading for lateral control */
        guidance_lat_adjust_heading(in_flight);
        /* set final attitude setpoint */
        stabilization_attitude_set_earth_cmd_i(&guidance_h_cmd_earth,
                                               guidance_lat_cmd_heading);
...
static void guidance_lat_adjust_heading(bool_t in_flight) {
  ...
  int32_t heading_correct = guidance_h_pos_err.y*guidance_lat.gains.p*GL_GAIN_SCALE/(1<<(GL_GAIN_FRAC+INT32_POS_FRAC-INT32_ANGLE_FRAC));
  ...
  guidance_lat_cmd_heading =  guidance_h.sp.heading + heading_correct; //in radians
}
